...
# Discord's gateway allows 5 presence updates per 20 seconds per session, so 10s (2 per
# 20s) sits well inside it. Was 60s, which made "rotating slowly" and "frozen" look
# identical for a full minute while this was being debugged.
STATUS_ROTATION_SECONDS = 10
...
def initial_activity() -> discord.Streaming:
    """The presence handed to discord.Bot(...) in bot.py, so the very first IDENTIFY
    already declares a streaming presence rather than the plain online Discord defaults
    to until the loop's first tick."""
    return discord.Streaming(name=random.choice(STATUS_LINES), url=STREAM_URL)


class StatusCog(commands.Cog):
    """Rotates the bot's presence through STATUS_LINES. Shuffled at startup so restarts
    don't always begin on the same line."""
    ...
    async def rotate_status(self):
        ...
        # except Exception, not HTTPException: tasks.loop dies for good after one uncaught
        # error (it did, live — the status rotated, then stopped forever), and
        # change_presence never touches REST, so HTTPException is one of the few things it
        # cannot raise. Naming that class here caught nothing at all.
        try:
            await self.bot.change_presence(status=PRESENCE_STATUS, activity=activity)
        except Exception:
            log.warning("Status rotation failed this cycle", exc_info=True)
            return

        # change_presence only affects the session that's already open. A *new* session
        # gets its presence from gateway.py's identify(), which reads these two fields —
        # and they're set once, from the discord.Bot(...) kwargs. So without this, every
        # reconnect that re-identifies snaps the presence back to the one line
        # initial_activity() picked at startup and leaves it there. That's what "stuck on
        # one line" was. Reaching into _connection because there is no public setter.
        state = self.bot._connection
        state._activity = activity.to_dict()
        state._status = str(PRESENCE_STATUS)

        # One INFO on the first tick is how the live log proves the loop is running at
        # all — a stuck presence looks identical whether the loop died, the gateway is
        # churning, or the Discord client just isn't re-rendering.
        if self.rotate_status.current_loop == 0:
            log.info("Status rotation live, every %ss", STATUS_ROTATION_SECONDS)
        log.debug("Status -> %s", activity.name)

    @rotate_status.before_loop
    async def before_rotate_status(self):
        # The cog-native version of starting the loop in on_ready, and safer: on_ready
        # fires again on every reconnect, and .start() raises if it's already running.
