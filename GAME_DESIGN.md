...
`services/ally_service.py`. `ALLY_NAMES = {"aunt_may": "Aunt May", "mj": "MJ"}`. Happiness (0–100) decays continuously (computed from `last_visited_at`, not a background tick) at `DECAY_PER_HOUR = 6.0` — deliberately fast: thriving (70+) erodes to neglected (<30) in under 7 hours of not visiting. Arachnid+ multiplies this by `1 + ARACHNID_ALLY_DECAY_INCREASE (0.3)` = 30% faster (§9.2 drawback).
...
- **Crime-level cost**: visiting raises city crime by `round(visit_seconds * CRIME_RISE_PER_SECOND (0.1))` — nobody's out patrolling while you're here.
